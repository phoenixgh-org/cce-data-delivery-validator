...
| `verdicts`                            | `object` — this transmission's verdict under each registered lineage, keyed by profile id: `{ "2025": "pass", "ds013": "fail" }`. Each value is `pass`, `fail`, or `null` (defined below); the shadow key is **absent**, not `null`, when no shadow lineage is registered. |
...
**`null` is narrower than "that lineage never ran".** A verdict is `null` only when
the lineage neither ran on the transmission nor inherits a failure from the other one
through the clause map. Transport and semantic breaches are graded once and shared
between the two lineages, so a transport halt reads as a failure under both, including
under the lineage whose validator never saw the body. The ordinary `null` is an
unresolved `meta.schemaVersion`: it raises a §3.2 failure, and §3.2 is the one
requirement whose DS01.3 counterpart is re-run rather than re-tagged, so the draft has
nothing to report.
