...
export interface ScopeTotals {
  ...
  /**
   * Distinct CCE units reported on in the scope (p98) — keyed on the appliance
   * manufacturer serial (ASER) where sent, otherwise the supplier's appliance
   * id (AMID); see src/api/scope.ts `unitTotals` for the key rule and its
   * schema citations. A count of what was RECEIVED, never fleet coverage
   * (DESIGN §7).
   */
  units: number;
