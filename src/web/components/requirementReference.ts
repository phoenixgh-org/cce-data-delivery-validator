...
export const REQUIREMENT_REFERENCE: Record<string, RequirementReference> = {
  ...
  '3.1': {
    ...
    guidance:
      'Structural conformance — the transmission metadata block and the DS01 object shapes — is graded under §3.2, where it is checked against the official published schema for the declared schemaVersion; we do not grade it twice. What we grade here is the half a schema cannot express: manufacturer-specific data objects — the lower-case "z"-prefixed names clause 4.5 requires (e.g. ztpcm), plus any object name that is neither a DS01 code nor a mis-cased one and therefore reads as custom by elimination (e.g. customTemp, zTPCM) — must be described by a schema carried in meta.customDataSchema. Send custom objects without it and this fails; send none and it passes as not applicable. We record the declaration only — the referenced schema is never fetched and your custom objects are never validated against it. We also note, informationally, object names that depart from the DS01 naming convention.',
  },
  '3.2': {
    ...
    guidance:
      'This is the core check — each body is validated against the official published schema for its declared schemaVersion. Open a failing transmission in the detail pane to see the exact JSON Pointer for each error.',
  },
  '3.3': {
