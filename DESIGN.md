...
- Advisories cost almost no DDL. `severity` is always `info`, and the identifier lives
  ...
  decision rather than a registration. `adv.identifier_disagreement` (7yuv) reads the
  same three identifiers as the second of them and adds no table at all, because a
  disagreement between two reports of one body needs no cross-transmission state.
...
| `adv.identifier_disagreement`   | Two reports of ONE transmission carrying the same appliance-side identifier beside different companion identifiers, over the same three pairs; both reports left the supplier's system together, which says that system held both versions and does not say which reading applies                | [`identifier-disagreement.ts`](src/ingest/stages/semantic/identifier-disagreement.ts) |
...
- `transmission_unit_window`: `transmission_id` (foreign key to `transmission`),
  ...
  whose records carry no parseable `ABST`, writes no row, and neither does a delivery
  the service did not accept: a delivery we refused is not one the next delivery is
  compared against. A 400 or a 413 leaves no parsed body to draw a window from; a 422
  parses first and is rejected by the schema stage afterwards, so its windows are real
  and are skipped deliberately. `findPriorUnitWindows` filters on `schema_ok IS TRUE`
  besides, so a row an older build wrote for a rejected body can never become a prior.
  The rows carry no independent lifetime: `ON DELETE CASCADE` from `transmission`
