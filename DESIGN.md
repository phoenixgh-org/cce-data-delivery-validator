...
| `adv.identifier_disagreement`   | Two reports of ONE transmission carrying the same appliance-side identifier beside different companion identifiers, over the same three pairs; both reports arrived together, so an identifier corrected between deliveries is not an available reading                                          | [`identifier-disagreement.ts`](src/ingest/stages/semantic/identifier-disagreement.ts) |
